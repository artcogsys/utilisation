...
from relational_model import RelationalResNet

flags = tf.app.flags
FLAGS = flags.FLAGS

flags.DEFINE_integer('batch_size', 200, 'Size of each training batch')
flags.DEFINE_bool('relational', True, 'if the same weights should be used for all residual blocks')
...
class Evaluation:
    def __init__(self, checkpoint):
        ...
        with self.graph.as_default():
            self.model = RelationalResNet(batch_size=FLAGS.batch_size, num_output_classes=10, placeholder_inputs=False,
                                          relational=FLAGS.relational)
...
def main(_):
    ...
    for checkpoint in checkpoints:
        e = Evaluation("checkpoints_tied/" + checkpoint)
        e.evaluate()
